...
def train_seg(net, train_data=None, train_labels=None, train_files=None,
    ...
    scale_range = 0.5 if scale_range is None else scale_range # [0.75, 1.25]
    ...
    for iepoch in range(n_epochs):
        ...
        for k in range(0, nimg_per_epoch, batch_size):
            ...
            with torch.autocast(device_type=device.type, dtype=net.dtype):
                y = net(X)[0]
                # len(y), y[0].shape: 1 torch.Size([3, 256, 256])
...
def _process_unlabeled(train_data=None,
                       train_probs=None, normalize_params={"normalize": False},
                       channel_axis=None, device=None):
    """
    Process train and test data.
    ...
    return train_data, train_probs, diam_train, normed


def _get_batch_unlabeled(inds, data=None, channel_axis=None, normalize_params={"normalize": False}):
    """
    Get a batch of images and labels.
    ...
    imgs = [data[i] for i in inds]
    imgs = _reshape_norm(imgs, channel_axis=channel_axis,
                         normalize_params=normalize_params)
    return imgs


def _loss_unlabeled(y, y_noisy):
    """
    Calculates the loss function between true labels lbl and prediction y.

    Args:
        y (torch.Tensor): Predicted values (flowsY, flowsX, cellprob).
        y_noisy (torch.Tensor): Predicted values (flowsY, flowsX, cellprob).
    ...
    loss = criterion(y[:, -3:-1], y_noisy[:, -3:-1])
    loss /= 2.

    loss2 = criterion2(y_noisy[:, -1], (y[:, -1] > 0.5).to(y.dtype))
    loss = loss + loss2
    return loss


def semi_train_seg(net, train_data=None,
                   channel_axis=None,
                   load_files=True, batch_size=1, learning_rate=1e-5, SGD=False,
                   n_epochs=100, weight_decay=0.1, normalize=True, compute_flows=False,
                   save_path=None, save_every=100, save_each=False, nimg_per_epoch=None,
                   rescale=False, scale_range=None, bsize=256,
                   model_name=None, class_weights=None):
    """
    Train the network with images for segmentation.
    ...
    out = _process_unlabeled(train_data=train_data, channel_axis=channel_axis,
                             normalize_params=normalize_params, device=net.device)
    train_data, train_probs, diam_train, normed = out
    ...
    train_logger.info(f">>> n_epochs={n_epochs}, n_unlabeled={nimg}")
    train_logger.info(
        f">>> AdamW, learning_rate={learning_rate:0.5f}, weight_decay={weight_decay:0.5f}"
    )
    optimizer = torch.optim.AdamW(net.parameters(), lr=learning_rate,
    ...
    for iepoch in range(n_epochs):
        ...
        for k in range(0, nimg_per_epoch, batch_size):
            kend = min(k + batch_size, nimg_per_epoch)
            inds = rperm[k:kend]
            imgs = _get_batch_unlabeled(inds, data=train_data, channel_axis=channel_axis,
                                        normalize_params=normalize_params)
            ...
            rsc = diams / net.diam_mean.item() if rescale else np.ones(
                len(diams), "float32")
            # augmentations
            imgi = random_rotate_and_resize(imgs, Y=None, rescale=rsc,
                                            scale_range=scale_range,
                                            xy=(bsize, bsize))[0]
            ...
            nchan_noise = 1
            diam_mean = 30
            random_diam = diam_mean * (2 ** (2 * np.random.rand(len(imgs)) - 1))
            imgi_noise = imgi.copy()
            imgi_noise = torch.from_numpy(imgi_noise).to(device)
            imgi_noise = torch.clamp(imgi_noise, 0.)
            imgi_noise[:, :nchan_noise] = add_noise(
                imgi_noise[:, :nchan_noise], poisson=0.7, blur=0.7, ds_max=7, iso=True,
                downsample=0.0, beta=0.7, gblur=1.0,
                diams=torch.from_numpy(random_diam).to(device).float())
            imgi_noise = imgi_noise.cpu().numpy()
            # end adding noise to images ------------------------------------------------------------------------------------

            # network and loss optimization
            # clean image
            X = torch.from_numpy(imgi).to(device)
            # noisy image
            X_noisy = torch.from_numpy(imgi_noise).to(device)

            with torch.autocast(device_type=device.type, dtype=net.dtype):
                with torch.no_grad():
                    y = net(X)[0]
                # len(y), y[0].shape: 1 torch.Size([3, 256, 256])
                y_noisy = net(X_noisy)[0]

            # semi_loss
            loss = _loss_unlabeled(y.detach(), y_noisy)
        ...
        if iepoch == 5 or iepoch % 10 == 0:
            lavg /= nsum
            train_logger.info(
                f"{iepoch}, unlabeled_train_loss={lavg:.4f}, LR={LR[iepoch]:.6f}, time {time.time() - t0:.2f}s"
            )
            lavg, nsum = 0, 0
