...
### random_rotate_and_resize is random crop !!!!!
def random_rotate_and_resize(X, Y=None, scale_range=1., xy=(224, 224), do_3D=False,
                             ...
                             random_per_image=True, center_override=None):
    ...
    for n in range(nimg):
        ...
        if random_per_image or n == 0:
            ...
            if center_override is not None:
                # Compute dxy to align crop center with foreground pixel (cx, cy)
                cx, cy = center_override
                cos_t = np.cos(theta)
                sin_t = np.sin(theta)
                # Reverse-compute dxy: dxy = -A @ ([cx,cy] - [Lx/2, Ly/2]), where A = scale * rotation
                dx = -scale[n] * (cos_t * (cx - Lx/2) - sin_t * (cy - Ly/2))
                dy = -scale[n] * (sin_t * (cx - Lx/2) + cos_t * (cy - Ly/2))
                dxy = np.array([dx, dy])
                # Clip to valid range to prevent out-of-bounds
                max_dxy = np.maximum(0, np.array([Lx * scale[n] - xy[1],
                                                  Ly * scale[n] - xy[0]]))
                dxy = np.clip(dxy, -max_dxy/2, max_dxy/2)
            else:
