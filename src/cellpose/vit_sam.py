...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        # print('input shape:', x.shape) # [256, 256]
        ...
        # print('before self.out:', x.shape)

        # readout is changed here
        x1 = self.out(x)
        # print('after self.out:', x1.shape)

        x1 = F.conv_transpose2d(x1, self.W2, stride = self.ps, padding = 0)
        # print('final:', x1.shape)

        # before self.out: torch.Size([64, 256, 32, 32])
        # after self.out: torch.Size([64, 192, 32, 32])
        # final: torch.Size([64, 3, 256, 256])
