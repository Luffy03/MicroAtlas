from cellpose import denoise, io, metrics, models
import time
...
def eval_subsets(root, dataset_name, seed=0, ntrain=1,
                 ...
                 device=torch.device("cuda")):
    """
    Evaluate Cellpose3 model performance on a single dataset
    ...
    netstr = f"cellpose3_cyto3_seed_{seed}_ntrain_{ntrain}"
    ...
    # # Select channel configuration based on dataset type (consistent with paper analysis.py)
    # cellpose must be [2, 1] !!!!
    if dataset_name == "Cellpose":
        channels = [2, 1]
    ...
    # Use Cellpose model for segmentation evaluation (consistent with paper cyto3_comparisons)
    model = models.Cellpose(gpu=True, model_type="cyto3")

    bsize = 224

    # Use paper's inference parameters: augment=True, tile_overlap=0.5, flow_threshold=0.4
    # diameter=0 lets the model auto-predict diameter

    out = model.eval(test_data, diameter=0, channels=channels,
                              tile_overlap=0.5, flow_threshold=0.4, augment=True,
                              batch_size=64, bsize=bsize, niter=2000)
    ...
    if save_results:
        ...
        save_dir = f"/data/linshan/Cellpose_data/all/predicted_masks_per_dataset/{dataset_name}_cellpose3_cyto3"
    ...
    result_txt = '/data/linshan/Cellpose_data/all/eval_results_cellpose3.txt'
...
if __name__ == "__main__":
    ...
    if args.save_results and len(all_dataset_names) > 0:
        ...
        excel_path = f'/data/linshan/Cellpose_data/all/cellpose3_cyto3.xlsx'
