"""
TNBC-MIBI Cellpose3 Evaluation Script
======================================
Segmentation → AP evaluation (AP@0.5/0.75/0.9) → Expression extraction + comparison → FlowSOM clustering
Usage: python TNBC-MIBI/analysis/eval_cellpose3_TNBC.py --gpu
"""
import sys, argparse, numpy as np, pandas as pd, tifffile, time
from pathlib import Path

# Cellpose3 requires cellpose-cp3 in sys.path and must be before the project root
# Ensure cellpose (CP3) overrides the system-installed cellpose (CP4)
_script_dir = Path(__file__).resolve().parent
_project_root = str(_script_dir.parent.parent)
sys.path.insert(0, _project_root)
_cp3_path = str(Path(_project_root) / 'cellpose-cp3')
...
def main():
    parser = argparse.ArgumentParser(description='TNBC-MIBI Cellpose3 Evaluation')
    ...
    parser.add_argument('--model_name', type=str, default='cellpose3')
    ...
    print(f"Cellpose3 Evaluation: {len(valid_pids)} patients, GPU={args.gpu}")
    ...
    if args.phase in ('all', 'segment'):
        ...
        for pid in valid_pids:
            ...
            # Composite RGB image → CP3 format (H,W) nuclear channel
            composite = tifffile.imread(str(COMPOSITE_DIR / f'Point{pid:02d}_composite.tif'))
            img_cp3 = format_for_cp3(composite)

            gt_path = MASK_DIR / f'p{pid}_labeledcellData.tiff'
            gt_mask = tifffile.imread(str(gt_path)).astype(np.uint16)

            masks_pred = model.eval([img_cp3], diameter=args.diameter,
                                    channels=[[1, 0]], niter=2000,
                                    batch_size=64, bsize=224,
                                    tile_overlap=0.5, augment=True)[0]
        ...
        lines = ["TNBC-MIBI Cellpose3 - AP Results",
    ...
    if args.phase in ('all', 'cluster'):
        ...
        if all_expr:
            ...
            result_df, per_patient_df = run_clustering(
                expr_concat, cell_df, gt_profiles, gt_counts, 'Cellpose3', out_dir, iou_thresh=args.iou_threshold)

            lines = ["TNBC-MIBI Cellpose3 - FlowSOM Cell-level Evaluation",
