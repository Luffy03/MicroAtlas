"""
TNBC-MIBI MicroAtlas Evaluation Script
========================================
Segmentation → AP evaluation (AP@0.5/0.75/0.9) → Expression extraction + comparison → FlowSOM clustering
Usage: python TNBC-MIBI/analysis/eval_microatlas_TNBC.py --gpu --weights ./microatlas/microatlas
"""
...
def main():
    parser = argparse.ArgumentParser(description='TNBC-MIBI MicroAtlas Evaluation')
    ...
    parser.add_argument('--model_name', type=str, default='microatlas')
    parser.add_argument('--weights', type=str, default=None,
                        help='MicroAtlas weights path')
    ...
    print(f"MicroAtlas Evaluation: {len(valid_pids)} patients, GPU={args.gpu}")
    ...
    if args.phase in ('all', 'segment'):
        ...
        for pid in valid_pids:
            ...
            # Composite RGB image → MicroAtlas format (3, H, W)
        ...
        lines = ["TNBC-MIBI MicroAtlas - AP Results",
    ...
    if args.phase in ('all', 'cluster'):
        ...
        if all_expr:
            ...
            result_df, per_patient_df = run_clustering(
                expr_concat, cell_df, gt_profiles, gt_counts, 'MicroAtlas', out_dir, iou_thresh=args.iou_threshold)

            lines = ["TNBC-MIBI MicroAtlas - FlowSOM Cell-level Evaluation",
