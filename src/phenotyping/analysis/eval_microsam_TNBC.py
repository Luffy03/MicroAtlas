...
def segment_with_microsam(image, model_type='vit_l_lm', device='cuda'):
    """Segment a single image with MicroSAM. Returns predicted mask (uint16)."""
    ...
    if prediction is not None and len(prediction) > 0:
        return np.asarray(prediction, dtype=np.uint16)
    else:
        return np.zeros(image.shape[:2], dtype=np.uint16)


def main():
    parser = argparse.ArgumentParser(description='TNBC-MIBI MicroSAM Evaluation')
    ...
    parser.add_argument('--model_name', type=str, default='microsam')
    ...
    print(f"MicroSAM Evaluation: {len(valid_pids)} patients, GPU={args.gpu}")
    ...
    if args.phase in ('all', 'segment'):
        ...
        lines = ["TNBC-MIBI MicroSAM - AP Results",
    ...
    if args.phase in ('all', 'cluster'):
        ...
        if all_expr:
            ...
            result_df, per_patient_df = run_clustering(
                expr_concat, cell_df, gt_profiles, gt_counts, 'MicroSAM', out_dir, iou_thresh=args.iou_threshold)

            lines = ["TNBC-MIBI MicroSAM - FlowSOM Cell-level Evaluation",
