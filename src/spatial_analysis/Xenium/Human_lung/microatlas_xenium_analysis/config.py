...
#   data      -> cpsam_xenium_analysis/../Xenium_V1_humanLung_Cancer_FFPE_outs/  (relative)
#   OR data   -> ../../../Xenium_V1_humanLung_Cancer_FFPE_outs/  (from cellpose repo root)
CONFIG_DIR = Path(__file__).parent.resolve()

# Try to locate the Xenium data directory relative to the project
_CANDIDATES = [
    CONFIG_DIR / ".." / "Xenium_V1_humanLung_Cancer_FFPE_outs",  # sibling dir
    CONFIG_DIR / ".." / ".." / ".." / ".." / "Xenium_V1_humanLung_Cancer_FFPE_outs",  # from cellpose repo
    CONFIG_DIR / ".." / ".." / "Xenium_V1_humanLung_Cancer_FFPE_outs",  # from Spatial_analysis/Xenium/Human_lung
]
...
HE_IMAGE_PATH = DATA_DIR / "Xenium_V1_humanLung_Cancer_FFPE_he_image.ome.tif"
...
LUNG_CELL_MARKERS = {
    "Alveolar": ["AGER", "PDPN", "SFTA2"],
    "Club_Ciliated": ["SCGB2A1", "FOXJ1", "SPDEF"],
    "Epithelial": ["EPCAM", "KRT7", "UPK3B"],
    "Endothelial": ["PECAM1", "VWF", "CAV1"],
    "Stromal": ["PDGFRA", "ACTA2", "MYH11"],
    "Immune": ["PTPRC", "CD3D", "CD68"],
    "Macrophage": ["CD14", "CD163", "MRC1", "FCGR3A", "TREM2"],
    "Lymphoid": ["MS4A1", "CD79A", "NKG7", "KLRD1", "MZB1"],
    "T_Cell": ["CD3E", "CD4", "CD8A", "GZMA", "GZMB"],
    "Mast": ["KIT", "MS4A2"],
    "Tumor_Epithelial": ["EPCAM", "ERBB2", "EGFR", "SOX2", "SNAI1"],
}
